...
// How long the connection must STAY offline before we show the blocking screen.
...
const configureNetInfo = () => {
  ...
  NetInfo.configure({
    ...
    // Any HTTP reply proves the internet reached the server, so even a 503
    // (server up, DB down) counts as reachable. Only a thrown/timed-out
    // request — i.e. no reply at all — means we are genuinely offline.
  ...
  const updateNetworkStatus = useCallback(
    (state: NetInfoState) => {
      ...
      if (!nextState.isOffline) {
        // Online (or recovered): cancel any pending offline flip and update instantly.
        clearOfflineTimer();
        setNetworkStatus(nextState);
        return;
      }

      // Offline: only surface the blocking screen if it PERSISTS past the debounce
      // window. A brief blip on resume clears before the timer fires.
